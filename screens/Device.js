...
const Device = ({ navigation, route }) => {
  const { ID, image, dname, desc, orderdata, email, screen } = route.params;
  ...
  useEffect(() => {
    ...
    //     `https://7549ddbe1a07.ngrok.io/api/getCompany/${Dtype}`
    //   );
  ...
  const getcompany = async () => {
    try {
      setLoading(true);
      const res = await axios.get(
        `http://realback4c.herokuapp.com/api/getCompany/${Dtype}`
      );
  ...
  const getmodel = async (itemValue) => {
    try {
      const res = await axios.get(
        `http://realback4c.herokuapp.com/api/getmodel/${itemValue}`
      );
